...
- AI 新写或修改 README、技术说明、Spec/Plan、issue/PR/release 文案、代码注释与 docstring 时，必须使用用户级 `$shuorenhua:shuorenhua` skill 做 `minimal` 自查：删模板腔、无源权威铺垫、渲染词堆砌和空洞收尾，同时保留事实、条件、术语与责任主体。
- 代码本体、配置、命令、路径、字段名、日志和报错不是风格改写对象；这些内容按 protected spans 原样保留。只有注释或说明文字可以按该 skill 调整，不能借改文风改变运行行为。
- reviewer 的 finding、结论及其审查对象里的文档、注释和其他面向用户文本都受同一约束。纯文风问题通常记为 Minor；只有造成歧义、事实失真、错误约束或不可执行时才按实际影响升级。
- 如果当前环境没有安装该 skill，明确说明缺失，不能声称已经完成这项检查，也不要临时复制或内嵌第三方 skill 内容。
